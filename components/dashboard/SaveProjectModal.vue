<template>
  <div v-if="isOpen" class="fixed left-0 top-0 z-[999] h-screen w-screen">
    ...
    <div
      ref="modalElement"
      class="absolute left-1/2 top-1/2 flex w-1/2 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center gap-5 rounded-xl bg-white p-5"
    >
      <div
        class="flex w-full items-center justify-between rounded-xl border-[0.5px] border-primary-500 bg-primary/20 px-3 py-1.5"
      >
        <div class="text-xl">Lagre prosjekt</div>
      ...
      <div class="flex w-full flex-col gap-3 px-3">
        <label>
          <p class="text-xs">Prosjekt navn</p>
          <CommonLineInput v-model="projectName" />
        </label>
        <label>
          <p class="text-xs">Prosjektbeskrivelse</p>
          <CommonTextArea v-model="projectDescription" />
        </label>
      </div>
